...
test("github collector reads releases and EIP commits (N+1)", async () => {
  const sources = {
    github: { release_watch: [{ owner: "ethereum", repo: "EIPs" }], eip_repo: { owner: "ethereum", repo: "EIPs" } },
  };
  const http = {
    getJson: async (url) => {
          ...
          json: [
            {
              ...
              body: "ERC-4337 paymaster fix",
            },
          ],
        };
      if (url.includes("/commits?path=EIPS")) return { json: [{ sha: "s1" }] };
      if (url.includes("/commits/s1"))
        return {
          json: {
            commit: { message: "Update ERC-7702", author: { name: "dev", date: "2026-06-20T00:00:00Z" } },
            files: [{ filename: "EIPS/eip-7702.md", status: "modified" }],
          },
        };
      return { json: [] };
    },
...
test("flashbots expander unpacks the newsletter into linked items", () => {
  const item = {
    source: "rss",
    subsource: "Flashbots MEV Newsletter",
    url: "https://collective.flashbots.net/t/the-mev-letter/1",
...
test("dedupAgainstPrevious drops items already covered by a prior file", () => {
  const item = {
    ...
    title: "ERC-4337 update",
    text: "x",
    metadata: { eip_numbers: [4337] },
  };
  const prev = [
    { file: "2026-06-20.md", content: "see https://blog.ethereum.org/aa", lower: "see https://blog.ethereum.org/aa" },
  ];
