#!/usr/bin/env python3
"""
telegram_fetch.py — the only Python in Scout. Telethon (MTProto) has no Node
equivalent, so the telegram collector (src/collectors/telegram.mjs) shells out
here. Fetches recent messages from the configured Telegram groups/channels and
prints them as a JSON array to stdout.

Behaviour:
- JSON to stdout only (the collector consumes stdout)
- Adds message_datetime_iso for a clean created_at downstream
- Never prompts: if the session is not authorized it exits cleanly with [] (safe for cron)
- Session path is explicit (--session) so there is no dependency on cwd

Runtime dependencies: telethon, python-dotenv. Install into the venv referenced
by config/sources.json → telegram.python_bin (see README "Telegram setup").

Credentials (read from environment, typically sourced from
~/.config/social-scan/.env): TELEGRAM_API_ID, TELEGRAM_API_HASH.
"""
import argparse
import asyncio
