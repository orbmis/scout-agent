...
export async function runDoctor(config) {
  ...
  add(
    "rss_feeds",
    deadFeeds.length === 0,
    `${feeds.length - deadFeeds.length}/${feeds.length} reachable${deadFeeds.length ? `; dead: ${deadFeeds.join(", ")}` : ""}`,
    "update the feed URL(s) in config/sources.json"
  );
  ...
  add(
    ...
    rl.ok && remaining > 0,
    rl.ok
      ? `${remaining} core requests remaining${secrets.GITHUB_TOKEN ? "" : " (unauthenticated — set GITHUB_TOKEN)"}`
      : "rate_limit endpoint unreachable",
    "set GITHUB_TOKEN in ~/.config/social-scan/.env"
  );

  // arxiv reachable
  const ax = await getText(
    `${sources.arxiv?.base_url || "https://export.arxiv.org/rss"}/${sources.arxiv?.categories?.[0] || "cs.CR"}`,
    { ua: "scout-doctor", timeoutMs: 10000 }
  );
  ...
  if (secrets.X_BEARER_TOKEN) {
    const x = await getText(`https://api.x.com/2/lists/${sources.x.list_id}/tweets?max_results=5`, {
      headers: { Authorization: `Bearer ${secrets.X_BEARER_TOKEN}` },
      timeoutMs: 10000,
    });
    add(
      "x",
      x.ok,
      x.ok ? "List readable" : `status ${x.status} — List may be private or token invalid`,
      "make the X List public and verify X_BEARER_TOKEN"
    );
  } else {
    add("x", false, "no X_BEARER_TOKEN (collector will be skipped)", "set X_BEARER_TOKEN to enable the X collector");
  }

  return { command: "doctor", ok: checks.every((c) => c.ok || c.name === "x"), checks };
}
