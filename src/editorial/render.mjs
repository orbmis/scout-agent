...
function renderEntry(entry) {
  ...
  if ((engagement.seed_engaged_by || []).length > 0)
    engagementParts.push(`seed engaged by ${engagement.seed_engaged_by.join(", ")}`);
  if ((engagement.likes || 0) + (engagement.reposts || 0) + (engagement.replies || 0) + (engagement.quotes || 0) > 0) {
    engagementParts.push(
      `${engagement.likes || 0} likes, ${engagement.reposts || 0} reposts, ${engagement.replies || 0} replies, ${engagement.quotes || 0} quotes`
    );
  }
  const lines = [
    `### ${titleFor(item)}`,
...
export function renderDaily({ signalDate, manifest, manifestPath, kept }) {
  ...
  const lines = [
    ...
    renderTier(
      ...
      `No Tier 0 items were surfaced. RSS kept ${diag.rss?.items_kept ?? 0} items and GitHub kept ${diag.github?.items_kept ?? 0}, but none cleared the editorial threshold after scoring.`
    ),
    renderTier(
      tiers,
      1,
      "Tier 1 — Seed-Set Signal",
      `No Tier 1 items were surfaced. X seed kept ${diag.x_seed?.items_kept ?? 0} items, but none cleared threshold after substance checks.`
    ),
    renderTier(
      tiers,
      2,
      "Tier 2 — Seed-Adjacent Signal",
      `No seed-adjacent items were surfaced. Collection kept ${diag.x_seed?.items_kept ?? 0} X seed items, but no non-seed author in today's manifest carried usable seed-engagement context.`
    ),
    renderTier(
      tiers,
      3,
      "Tier 3 — Independent Signal",
      `No independent candidates cleared into the final note. Today's manifest included ${diag.arxiv?.items_kept ?? 0} arXiv items after collection-time filtering.`
    ),
  ...
  if (kept.length) {
    ...
    lines.push(
      "Today's manifest skewed heavily toward seed-set chatter and low-signal newsletter items rather than concrete standards, infrastructure, or payment developments."
    );
  }
  lines.push("");
  return lines.join("\n");
...
export function renderFiltered({ signalDate, kept, filtered }) {
  ...
  for (const klass of ["below_threshold", "missing_anchor_signal", "topic_dedup", "collapsed_to_cluster"]) {
    if (klass !== "below_threshold") {
      lines.push(
        `## ${klass === "missing_anchor_signal" ? "Missing required anchor signal" : klass === "topic_dedup" ? "Topic dedup / prior coverage" : "Collapsed to cluster"}`,
        ""
      );
    }
    ...
    for (const entry of items) {
      ...
      if (item.created_at) lines.push(`- **Captured timestamp:** ${item.created_at}`);
      lines.push(
        `- **Exclusion class:** ${entry.exclusionClass}`,
        `- **Reason:** ${entry.reason}`,
        `- **Scoring note:** ${entry.composite} score; dominant axis ${entry.dominantAxis}.`,
        ""
      );
    }
  }
