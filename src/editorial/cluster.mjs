...
export function topicKeyFor(item) {
  ...
  const parts = [];
  const matchedEips = Array.from(
    text.matchAll(/(?:eip|erc)[- ]?(4337|7702|7579|7710|7715|7521|7683|8211|7928|8264|8253|7773)/g)
  ).map((m) => m[1]);
...
export function dedupAgainstPrevious(item, previousEntries) {
  ...
  const eips = new Set([
    ...
    ...Array.from(
      (title + " " + text).matchAll(/(?:eip|erc)[- ]?(4337|7702|7579|7710|7715|7521|7683|8211|7928|8264|8253|7773)/gi)
    ).map((m) => m[1]),
  ]);
